...
def flush_messages(room_id: str):
    ...
    # 응답 생성중 표시
    asyncio.create_task(
        sio.emit("saying", {}, room=room_id, namespace="/rag")
    )
...
@sio.on("join_room", namespace="/rag")
async def handle_join_room(sid, data):
    ...
    await sio.enter_room(sid, room_id, namespace="/rag")
    ...
    print(f"🔍 현재 {sid}의 Room 리스트: {sio.rooms(sid, namespace='/rag')}")
    ...
    # 클라이언트에게 알림
    await sio.emit("room_joined", {"room_id": room_id}, room=sid, namespace="/rag")

@sio.on("typing_start", namespace="/rag")
async def handle_typing_start(sid, data):
    ...
    await sio.emit("typing_indicator", {
        ...
        "nickname": nickname,
        "typing": True
    }, room=room_id, skip_sid=sid, namespace="/rag")

@sio.on("typing_stop", namespace="/rag")
...
@sio.on("chat_message", namespace="/rag")
async def handle_chat_message(sid, data):
    ...
    # await sio.emit("saying", {}, room=room_id, namespace="/rag")

# --- 아래는 WebRTC signaling 이벤트 추가 부분 ---
@sio.on("offer", namespace="/rag")
async def handle_offer(sid, data):
    ...
    await sio.emit("offer", data, room=room_id, skip_sid=sid, namespace="/rag")

@sio.on("answer", namespace="/rag")
async def handle_answer(sid, data):
    ...
    await sio.emit("answer", data, room=room_id, skip_sid=sid, namespace="/rag")

@sio.on("ice-candidate", namespace="/rag")
async def handle_ice_candidate(sid, data):
    ...
    await sio.emit("ice-candidate", data, room=room_id, skip_sid=sid, namespace="/rag")
...
async def chat_close(request: CloseChatRequest):
