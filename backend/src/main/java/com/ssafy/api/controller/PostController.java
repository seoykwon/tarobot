...
public class PostController {
    ...
    // **2. 모든 게시글 조회**
    @GetMapping
    @ApiOperation(value = "모든 게시글 조회", notes = "활성화된 모든 게시글을 조회합니다.")
    @ApiResponses({
            @ApiResponse(code = 200, message = "성공"),
            @ApiResponse(code = 500, message = "서버 오류")
    })
    ...
    @ApiOperation(value="게시글 삭제 (비활성화)", notes="특정 게시글을 비활성화 처리합니다.")
    @ApiResponses({
    ...
    public ResponseEntity<Void> deactivatePost(
            @PathVariable @ApiParam(value="게시글 ID",required=true) Long postId,
            // 현재 사용자 정보는 SecurityContextHolder 등을 통해 가져올 수 있음
            // 여기서는 간단히 User 객체를 직접 전달받는 것으로 가정
            User currentUser) {
