...
def make_prompt_ner(text: str) -> str:
    ...
    용언을 keyword로 추출할 때는 명사형으로 입력할 것.
...
def make_prompt_timestamp(user_query: str) -> str:
    ...
    변환 로직에서 UTC → KST 변환이 처리되므로, 응답은 반드시 UTC 기준이어야 합니다.  
    날짜가 중요하지 않다면 `"use_as_filter": false"`를 반환하세요.

    ✅ 날짜가 핵심적인 경우 → `true` (검색 필터 적용)  
    ✅ 단순한 배경 정보 또는 트렌드 언급 → `false` (검색 필터 제외)  
